...
const ChartWrapper = ({expensesList}) => {
  ...
  const totalEntertainmentExpenses = expensesList.reduce((total, expenseObj) => {
  ...
  const chartData = {
    ...
    datasets: [
      {
        data: [totalFoodExpenses, totalClothesExpenses, totalTransportExpenses, totalMedicineExpenses, totalEntertainmentExpenses], 
        backgroundColor: ['#8871fb', '#c771fb', '#f971f0', '#a94064', '#7aa8fb' ],
  ...
  return (
    <div className={`chart-wrapper ${isPopupVisible ? 'blur-effect' : ''}`}>
      <DoughnutChart data={chartData} onClick={handleChartClick} chartOptions={chartOptions}/>
      {isPopupVisible && (
        <Popup data={selectedChartData} onClose={handlePopupClose} chartOptions={chartOptions}/>
      )}
    </div>
  );
};
